...
- Frontier settings are active as `openai-codex` / `gpt-5.5` through Codex OAuth, with 1,000,000 context and compression enabled at `0.25`.
...
- The local Tolui endpoint is managed by a private macOS LaunchAgent and is configured as Cerberus's automatic fallback when the primary provider is rate-limited; a fallback one-shot passed.
